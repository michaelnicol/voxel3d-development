...
export class BaseObject {
    ...
    */
    ...
    static convertPointsToDemos(points) {
    ...
    */
    ...
    findPointFromObject(point) {
        ...
        let asseretedTSBoundingBox = this.boundingBoxMeta;
        let targetKey = point[asseretedTSBoundingBox.biggestRangeIndex["" + 0]];
...
export class LayerConvexExtrude extends BaseObject {
     ...
     */
    static polarSort(inputPoints, center) {
    ...
    static convexHull(inputPoints) {
        ...
        let sortedPoints = BaseObject.push2D(inputPoints, []).sort((a, b) => a[0] - b[0]);
    ...
    generateEdges(includeShapeEdges) {
        ...
        for (let i = 0; i < this.extrudeObjects.length; i++) {
            if (i + 1 < this.extrudeObjects.length) {
                ...
                if (includeShapeEdges) {
                    ...
                    sortedFillEntry = sortedFillEntry.map((v) => v.filter((coordValue, index) => index != boundingBoxMetaReference.biggestRangeIndex[0]));
                    sortedFillEntry = LayerConvexExtrude.convexHull(sortedFillEntry);
                    sortedFillEntry = sortedFillEntry.map((v) => { return v.splice(boundingBoxMetaReference.biggestRangeIndex[0], 0, Number(sectionKeys[0])), v; });
                    edgeVoxel.delete();
                    ...
                    sortedFillEntry = sortedFillEntry.map((v) => v.filter((coordValue, index) => index != boundingBoxMetaReference.biggestRangeIndex[0]));
                    sortedFillEntry = LayerConvexExtrude.convexHull(sortedFillEntry);
                    sortedFillEntry = sortedFillEntry.map((v) => { return v.splice(boundingBoxMetaReference.biggestRangeIndex[0], 0, Number(sectionKeys[sectionKeys.length - 1])), v; });
        ...
        this._fillVoxels = this.getEdgeDirectory(LayerConvexExtrudeEdgeDirectoryOptions.RETURN_MODE_VOXELS);
        this.calculateBoundingBox();
        return this;
    }
